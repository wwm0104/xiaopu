...
function initPartnerData(page, row) {
    ...
    $.ajax({
        processData: false,
        type: 'POST',
        url: "/partnerManage/getSchoolPartnerLook",
...
function success()
{
    ...
    $.ajax({
        processData: false,
        type: 'POST',
        url: "/partnerManage/checkSchoolPass",
        contentType: 'application/json',
        data: JSON.stringify({id: id,status: status,starRating:starRating,remarks:remarks,sex:sex,schoolId:schoolId,schoolName:schoolName}),
        dataType: 'json',
        success: function (data) {
...
function getSchoolList(){
    $.ajax({
        ...
        success: function(data){
            $('#schoolList').empty();
            var tdHtml = "";
            tdHtml +="<option value='-1'>所有大学名称</option>";
