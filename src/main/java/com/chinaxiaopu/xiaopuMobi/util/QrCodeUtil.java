...
public class QrCodeUtil {
    ...
    private static final String LOGO_IMG_PATH = "group.png";
    /**
     * 生成带logo的二维码图片
     *
     * @param qrUrl  待编码URL
     * @param desc 二维码文字
     * @param qrPath 生成二维码存放路径，带名称
     */
    public static String getLogoQRCode(String qrUrl, String desc, String qrPath) {
        //filePath是二维码logo的路径，但是实际中我们是放在项目的某个路径下面的，所以路径用上面的，把下面的注释就好
        String content = qrUrl;
        try {
            //TOOD 重构
            String filePath = SystemConfig.getInstance().getImgBasePath()+LOGO_IMG_PATH;
            ...
            return zp.addLogo_QRCode(bim, new File(filePath), new LogoConfig(), desc,qrPath);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
     ...
     * @param logoPic
     */
    public String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName, String qrPath) {
        try {
            /**
             * 读取二维码图片，并构建绘图对象
             */
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
            ...
            ImageIO.write(image, "png", new File(qrPath+".png")); //TODO
